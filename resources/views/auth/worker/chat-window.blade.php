<x-worker-dashboard>
    ...
    <div class="mt-4 bg-white rounded-lg shadow-md p-6"
        x-data="{{ json_encode(['messages' => $messages, 'messageBody' => '','client_id'=> $client_id,'own_user_id' => $own_user_id]) }}"
        x-init="
        ...
        <div class="flex justify-start items-center p-1 ">
            <div class="relative">
                <input
                    x-on:keydown.enter="sendMessage('{{ route('worker.chat.post') }}','{{ csrf_token() }}',messages, messageBody,client_id);messageBody = '';  "
                    x-model="messageBody"
            ...
            <div class="w-7 h-7 rounded-full ml-2 bg-blue-300 text-center items-center flex justify-center">
                <button
                    ...
                    x-on:click="sendMessage('{{ route('worker.chat.post') }}','{{ csrf_token() }}',messages,messageBody,client_id);messageBody = '';">
